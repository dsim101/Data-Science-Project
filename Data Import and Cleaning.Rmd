...
# minVal <- min(var)
 #maxVal <- max(var)
#val_array <- minVal:maxVal

reverseCode <- function(var) {
  revalue(var, c("7"="1", "6"="2", "5"="3", "4"="4", "3"="5", "2"="6", "1"="7"))
}

data$SDO2 <- reverseCode(data$SDO2)

revSdoItems <- data %>% select("SDO1":"SDO8")
test <- as.data.frame(revSdoItems)
  
for(x in 1:length(revSdoItems)) {
  reverseCode(revSdoItems[x]) 
}
